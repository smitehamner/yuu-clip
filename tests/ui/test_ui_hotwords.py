...
def _create_hotword(page: Page, phrase, match_mode="exact", boost=0.2, target="funny", enabled=True, context_slug=None):
    return page.evaluate(
        """(args) => fetch('/api/hotwords', {
            method: 'POST', headers: {'Content-Type': 'application/json'},
            body: JSON.stringify(args),
        }).then(r => r.json())""",
        {"phrase": phrase, "match_mode": match_mode, "boost": boost, "target": target,
         "enabled": enabled, "context_slug": context_slug},
    )
...
@skip_no_server
class TestHotwordContextScopeUI:
    def test_global_hotword_renders_under_global_group(self, page: Page):
        hw = _create_hotword(page, "uitest_global_scope")
        try:
            _open_settings(page)
            row = page.locator(f'[data-hotword-row="{hw["id"]}"]')
            expect(row.locator(".hw-context")).to_have_value("")
            expect(page.locator("#s-hotword-rows")).to_contain_text("Global (all recordings)")
        finally:
            _delete_hotword(page, hw.get("id"))

    def test_context_hotword_renders_selector_and_group_heading(self, page: Page):
        hw = _create_hotword(page, "uitest_ctx_scope", context_slug="fantasy-rp")
        try:
            _open_settings(page)
            row = page.locator(f'[data-hotword-row="{hw["id"]}"]')
            expect(row.locator(".hw-context")).to_have_value("fantasy-rp")
            # The built-in context's display name is the group heading.
            expect(page.locator("#s-hotword-rows")).to_contain_text("Fantasy RP")
        finally:
            _delete_hotword(page, hw.get("id"))

    def test_changing_scope_persists_via_put(self, page: Page):
        hw = _create_hotword(page, "uitest_scope_change")
        try:
            _open_settings(page)
            row = page.locator(f'[data-hotword-row="{hw["id"]}"]')
            row.locator(".hw-context").select_option("multiplayer-shooter")
            expect(page.locator("#toast-container .toast.success")).to_contain_text("Hot-word saved")
            saved = page.evaluate(
                "(id) => fetch('/api/hotwords').then(r => r.json())"
                ".then(list => list.find(h => h.id === id))",
...
@skip_no_server
class TestRescanHotwordsMenuAction:
    """UX-M7: 'Rescan Hot-words' is a durable Additional Actions menu row, not
    only a one-shot toast shortcut a user can miss right after saving."""
