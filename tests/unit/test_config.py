"""Config: pure Config.load()/save round-trips, validators, and profile CRUD.

Client-bound /api/config route tests live in tests/integration/test_config.py."""
...
# ---------------------------------------------------------------------------
# Config.load() sanitization of hand-edited/stale values (moved out of the
# client-bound TestUiConfig/TestCaptionStyleConfig/TestVisionConfig classes -
# these particular tests use only tmp_path/monkeypatch)
# ---------------------------------------------------------------------------

def test_load_sanitizes_unknown_content_preset(tmp_path, monkeypatch):
...
def test_load_coerces_retired_pyannote_backend(tmp_path, monkeypatch):
...
def test_load_sanitizes_bad_values(tmp_path, monkeypatch):
    import json as _json
...
def test_load_sanitizes_bad_chunk_size(tmp_path, monkeypatch):
    import json as _json
...
def test_load_sanitizes_bad_frame_count(tmp_path, monkeypatch):
    import json as _json
...
# ---------------------------------------------------------------------------
# whisper_language - config field and pipeline resolution
# ---------------------------------------------------------------------------
