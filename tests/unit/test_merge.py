...
highest motion peak first, with deterministic output ordering.
"""
...
def test_visual_fully_inside_transcript_is_dropped():
    ...
    merged = merge_candidates(transcript, visual, Config())
    assert merged == transcript  # visual deduped out, transcript wins
...
def test_overlap_above_threshold_dropped():
    ...
    merged = merge_candidates(transcript, visual, cfg)
    assert visual[0] not in merged
...
def test_overlap_below_threshold_keeps_both():
    ...
    merged = merge_candidates(transcript, visual, cfg)
    assert transcript[0] in merged
    assert visual[0] in merged
...
def test_cap_keeps_highest_peak_visual_only():
    ...
    merged = merge_candidates(transcript, visual, cfg)
    kept_peaks = sorted(c.visual_peak for c in merged)
...
def test_deterministic_ordering_by_start_ms():
    ...
    merged = merge_candidates(transcript, visual, cfg)
    visual_part = [c for c in merged if c is not transcript[0]]
    assert [c.start_ms for c in visual_part] == [100_000, 200_000, 300_000]


def test_empty_visual_returns_transcript_unchanged():
    transcript = [_cand(0, 20_000), _cand(30_000, 50_000)]
    merged = merge_candidates(transcript, [], Config())
