...
Schema evolution across an app update used to live here (the _ADDITIVE_COLUMNS
ALTER-on-open list); it is now Alembic's job - the baseline + startup auto-migrate
(yuu_clip/db/migrate.py), guarded by tests/unit/test_migration_drift.py (schema stays
in sync with the models) and tests/unit/test_startup_migrate.py (the upgrade/backup
behavior). What remains here is the create_engine call-site invariants themselves.
"""
