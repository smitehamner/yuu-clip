...
Driven directly on a constructed AnalyzeJob / _replay_lines - no server, no
subprocess. Previously only reachable indirectly via
tests/integration/test_reattach.py's client + subprocess mocks."""
...
class TestReplayLines:
    def _marker(self, stage, done=None, total=None):
        from yuu_clip.pipeline.progress import format_progress
        return format_progress(stage, done=done, total=total)

    def _replay(self, buffer):
        from yuu_clip.web.analyze_job import _replay_lines
        return _replay_lines(buffer)

    def test_empty_buffer_replays_empty(self):
        assert self._replay([]) == []

    def test_short_plain_buffer_replays_in_full(self):
        buffer = [f"line {i}" for i in range(5)]
    ...
    def test_long_plain_buffer_replays_only_the_tail(self):
        ...
        buffer = [f"line {i}" for i in range(_REPLAY_TAIL_LINES + 20)]
    ...
    def test_old_marker_outside_the_tail_is_still_included(self):
        ...
        buffer = [marker] + [f"line {i}" for i in range(_REPLAY_TAIL_LINES + 20)]
    ...
    def test_marker_already_inside_the_tail_is_not_duplicated(self):
        from yuu_clip.web.analyze_job import _REPLAY_TAIL_LINES
        marker = self._marker("extract", done=1, total=2)
        buffer = [f"line {i}" for i in range(5)] + [marker]
    ...
    def test_only_the_latest_marker_per_stage_is_kept(self):
        ...
        buffer = [old_marker, new_marker] + [f"line {i}" for i in range(_REPLAY_TAIL_LINES + 20)]
        replay = self._replay(buffer)
    ...
    def test_one_marker_kept_per_distinct_stage(self):
        ...
        buffer = [extract_marker, transcribe_marker] + [f"line {i}" for i in range(_REPLAY_TAIL_LINES + 20)]
        replay = self._replay(buffer)
    ...
    def test_markers_kept_in_original_buffer_order(self):
        ...
        buffer = [transcribe_marker, extract_marker] + [f"line {i}" for i in range(_REPLAY_TAIL_LINES + 20)]
        replay = self._replay(buffer)
...
class TestAnalyzeJobEmit:
    ...
    def test_lines_within_cap_are_all_kept(self, tmp_path):
        ...
        for i in range(10):
            job._emit(f"line {i}")
        assert job.buffer == [f"line {i}" for i in range(10)]
    ...
    def test_buffer_trimmed_to_cap_when_exceeded(self, tmp_path, monkeypatch):
        ...
        assert job.buffer == [f"line {i}" for i in range(3, 8)]  # oldest 3 dropped
    ...
    def test_emit_with_no_subscribers_does_not_raise(self, tmp_path):
        job = self._job(tmp_path)
        job._emit("no one is listening")  # must not raise
        assert job.buffer == ["no one is listening"]
