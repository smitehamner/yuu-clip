...
class TestVisualBlock:
    ...
    def test_describe_clip_non_dict_response_raises_value_error(self):
        """describe_clip validates isinstance(dict) before calling .get(), matching
        find_related_clips/request_scene_boundaries's list-shape guards - a model
        that replies with a JSON array gets a clean ValueError, not AttributeError."""
        import json
        import unittest.mock as mock
