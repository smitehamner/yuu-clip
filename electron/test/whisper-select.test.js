...
test('recommends large-v3 at 6 GB+ VRAM', () => {
  assert.deepEqual(recommendWhisperModel(6000), { model: 'large-v3', reason: '6 GB+ VRAM - best accuracy' });
});

test('recommends medium just under the large-v3 boundary', () => {
  assert.deepEqual(recommendWhisperModel(5999), { model: 'medium', reason: '4 GB+ VRAM - good accuracy' });
});

test('recommends medium at the 4 GB boundary', () => {
  assert.deepEqual(recommendWhisperModel(4000), { model: 'medium', reason: '4 GB+ VRAM - good accuracy' });
});

test('recommends small just under the medium boundary', () => {
  assert.deepEqual(recommendWhisperModel(3999), { model: 'small', reason: '2 GB+ VRAM - balanced' });
});
