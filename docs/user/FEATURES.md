...
model (the bundled llama.cpp engine, the default) or the Claude API, chosen in the setup wizard or Settings.
When speaker labels
...
A larger or slower model multiplies these times proportionally.

For the local **llama.cpp** backend, **Use GPU when available** (Settings → LLM scoring, on by default) offloads the model to your graphics card. The bundled engine is GPU-accelerated on NVIDIA, AMD, and Intel GPUs out of the box, so scoring runs on the GPU with no extra steps; if the model is too large for your GPU's memory it spills to the CPU. Turn the setting off to force CPU.

#### Recommended models and readiness

Settings → LLM scoring lists a curated set of **recommended models** for each backend. Every recommended model is licensed so that clips you make with it can be monetized - Apache-2.0 or MIT for local models (llama.cpp), and the Anthropic API's commercial terms for Claude. Models under Meta's Llama licence or Google's Gemma terms are deliberately **left out** of the recommendations because their use restrictions would need a case-by-case legal reading; they still work if you configure them by hand.

Recommended models are split into two labelled groups - **Text scoring models** and **Image analysis (vision) models** - so it's clear what each is for. Each local `.gguf` model has a one-click **Download now** button (it fetches the file into your project's models folder and points scoring at it), plus **Use this model** for a file you already have; a vision entry downloads both its main model and its *vision projector* in one click. The model currently in use shows an **Active** badge, and the download page link stays available as a "choose a different file" fallback. A **Model readiness** line shows whether the model you've set up can score **text** and analyze **images** right now, and explains what's missing if not (for example, a Claude API key, or - for the local llama.cpp backend - a separate *vision projector* `.gguf` file, which is what enables image analysis).

For the local backend you set a **Text model** for scoring and, optionally, a separate **Vision model** (plus its projector) for image analysis - so a strong text model can handle scoring while a smaller vision model handles frames.
