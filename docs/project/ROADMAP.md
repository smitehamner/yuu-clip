...
- **No Playwright coverage for SSE happy paths** — the only safety net for the stream-based jobs (analyze, export, scoring) is the live-server `test-ui.ps1` suite, and it doesn't exercise their success flow; `test-api.ps1` can't reach the UI job state, so the Phase 3 stuck-job-UI bug slipped through both. Adding a Playwright test for at least one SSE job's happy path would close the gap.
