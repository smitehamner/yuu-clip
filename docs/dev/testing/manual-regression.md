...
- **Installed-app release checklist** (the manual walk, per use case, against the
  packaged build - including the packaged-only surface detail formerly in
  `packaged-app-verification.md`: install, environment bring-up, wizard, native
  `yuu-media://` protocol, model downloads, lifecycle):
  [installed-app-checklist.md](installed-app-checklist.md)
