...
A Person may hold at most **one Character link per world context** - the same voice can play a different character in a different context (e.g. one campaign vs. another), and each of those is scored independently. A clip only ever picks up the alias whose context is active for that clip's recording (`Video.context_names_json`, which can hold more than one context - "crossover sessions"); an alias from a context the recording isn't tagged with never leaks into scoring.

- **Code:** `Character` (`db/models.py`), keyed to a JSON context by `context_slug` (a plain string, not a FK - contexts live in `contexts.json`, same precedent as `HotWord.context_slug`); `PersonCharacterLink` / `person_characters` table (`ProjectVoice.character_links`, one row per Person per context, real FKs with `ON DELETE CASCADE`); routes in `web/routes/characters.py` (`/api/contexts/{slug}/characters`, `/api/characters*`) and `POST /api/voices/{id}/characters` (body `{context_slug, character_id}`, upserts within that context); prompt seam `contexts.format_character_block`; context-scoping via `scoring/term_scope.video_context_ids` in `scoring/llm.py::_characters_in_clip`; UI in `contexts.js` (editor section) + `voices.js` (one picker per world context per Person).
...
- **Notes:** Optional and additive - a clip with no linked Character produces a prompt byte-identical to a project with no characters. Deleting a Character (or its whole context) removes any Person's link to it in code (`ON DELETE CASCADE` is also a DB-level backstop); it never blocks the delete and never touches the Person's name or voiceprint. Merging two People transfers the source's aliases onto the target for any context the target doesn't already hold one in; a colliding context's alias is dropped with the merged-away Person rather than silently overwriting the target's choice.
