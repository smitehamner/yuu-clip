...
| `characters` (`Character`) | Structured lore entity in a world context (name, lore, score boost), aliasable to a Person | Character | Context editor (`routes/characters.py`) | LLM scoring prompt (`contexts.format_character_block`); per-context Person picker (`people/voices.js`) |
