...
yuu_clip/
  ...
  dev/                     # The yuu-dev developer-loop CLI (Typer, cross-platform): serve, status, logs, lint (lint.py), typecheck (typecheck.py, mypy + frozen baseline), the test runners (tests.py = test-unit/-integration/-api/-system/-golden/-ui/-all, testjs.py = test-js, chaos.py = test-chaos - manual exploratory Playwright bug-hunt against the fixture server, NOT a gate, excluded from CI and every test-* combo), bundle (bundle.py -> esbuild + the index.html stitch in htmlstitch.py), shared-data (shareddata.py), fixture-project (fixture.py), uiserver.py (the disposable fixture server test-ui/test-chaos stand up), help-docs (helpdocs.py), notices (notices.py), lock-deps (deps.py), _summary.py (pytest-output summary core), procs.py (Windows process reap; no-ops off Windows)
...
scripts/                   # Standalone tooling that isn't a yuu-dev subcommand: build-esm.mjs (esbuild driver behind `yuu-dev bundle`), screenshots.py (regenerates the README screenshots from a live dev server), select_ui_tests.py (maps changed files -> `tests/ui/test_ui_*.py` for `yuu-dev test-ui --changed`), test-system.ps1 / test-golden.ps1 (thin Windows wrappers over `yuu-dev test-system` / `yuu-dev test-golden`, kept for muscle memory - see CONTRIBUTING.md), windows-release/ (the packaged-build pipeline: fetch-wheelhouse.ps1, fetch-python-runtime.ps1, fetch-ffmpeg-runtime.ps1, fetch-llama-server-runtime.ps1, build-prebuilt-env.ps1, build-release.ps1, check_wheel_deps.py)
```
